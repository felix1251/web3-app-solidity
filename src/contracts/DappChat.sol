...
contract DappChat {
    ...
    function addChannelMembers(uint channelIndex, address newMember) public {
        ...
        require (newMember != msg.sender, "Alredy a member, Admin");
        require( allChannels[channelIndex].members[newMember] != true, "Already a user, Member");
    ...
    function unfollowChannel(uint indexToUnfollow) public {
        ...
        allChannels[indexToUnfollow].members[msg.sender] = false;
    } 

    function deleteChannel(uint indexTodelete) public {
        require( allChannels[indexTodelete].channelOwner == msg.sender, "Only the admin can delete this channel" ); 
        delete allChannels[indexTodelete]; 
    } 
    ...
    function ifInvited(uint index) public view returns (bool) {
        if( allChannels[index].members[msg.sender] == true ) 
        {    
            return true;
        }else{
            return false;
